import os
import tempfile
import shutil
import logging
from typing import List, Dict, Tuple
import uvicorn
from fastapi import FastAPI, File, UploadFile, HTTPException, BackgroundTasks
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import FileResponse
from moviepy.editor import VideoFileClip, AudioFileClip, CompositeAudioClip
import whisper
from transformers import MarianMTModel, MarianTokenizer
import azure.cognitiveservices.speech as speechsdk
import torch
from pathlib import Path
from datetime import datetime
import numpy as np
import librosa
import soundfile as sf
import noisereduce as nr
from pydub import AudioSegment
import webrtcvad
import warnings
import asyncio
from concurrent.futures import ThreadPoolExecutor
import uuid

warnings.filterwarnings("ignore", category=UserWarning, module="transformers")
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

AZURE_SPEECH_KEY = os.getenv("AZURE_SPEECH_KEY", "FMJPLiTea92XmK7ZNqv3CscieRTdQNU5ihZ26RFUrHACpxPKTLiMJQQJ99BFACgEuAYXJ3w3AAAYACOGCMRL")
AZURE_SPEECH_REGION = os.getenv("AZURE_SPEECH_REGION", "italynorth")

CONFIG = {
    "whisper_model": "tiny",
    "translation_model": "Helsinki-NLP/opus-mt-en-ar",
    "arabic_voice": "ar-SA-HamedNeural",
    "max_segment_gap": 0.5,
    "audio_quality": "high",
    "video_codec": "libx264",
    "audio_codec": "aac",
    "max_file_size": 500 * 1024 * 1024,
    "supported_formats": [".mp4", ".avi", ".mov", ".mkv", ".wmv", ".flv"],
    "temp_cleanup": True,
    "vocal_isolation_strength": 0.9,
    "noise_reduction_strength": 0.4,
    "audio_normalization": True,
    "sample_rate": 22050,
    "background_reduction_factor": 0.15
}

app = FastAPI(title="Video Dubbing API", description="API لدبلجة الفيديوهات من الإنجليزية إلى العربية", version="1.0.0")
app.add_middleware(CORSMiddleware, allow_origins=[""], allow_credentials=True, allow_methods=[""], allow_headers=["*"])
translator = None
executor = ThreadPoolExecutor(max_workers=2)

class VideoTranslatorError(Exception):
    pass

class EnhancedVideoTranslator:
    def __init__(self):
        self.temp_dir = tempfile.mkdtemp()
        device = "cuda" if torch.cuda.is_available() else "cpu"
        self.whisper_model = whisper.load_model(CONFIG["whisper_model"], device=device)
        self.translation_model = MarianMTModel.from_pretrained(CONFIG["translation_model"])
        self.tokenizer = MarianTokenizer.from_pretrained(CONFIG["translation_model"])
        try:
            self.vad = webrtcvad.Vad(2)
        except:
            self.vad = None

    def validate_input(self, video_path: str) -> bool:
        if not video_path or not os.path.isfile(video_path):
            raise VideoTranslatorError("Invalid video file")
        if os.path.getsize(video_path) > CONFIG["max_file_size"]:
            raise VideoTranslatorError("File too large")
        if Path(video_path).suffix.lower() not in CONFIG["supported_formats"]:
            supported = ', '.join(CONFIG["supported_formats"])
            raise VideoTranslatorError(f"Unsupported format. Supported: {supported}")
        return True

    def advanced_audio_separation(self, video_path: str) -> Tuple[str, str, str, float]:
        full_audio_path = os.path.join(self.temp_dir, "full_audio.wav")
        with VideoFileClip(video_path) as video:
            if video.audio is None:
                raise VideoTranslatorError("No audio track found")
            duration = video.duration
            video.audio.write_audiofile(full_audio_path, codec='pcm_s16le', verbose=False, logger=None)
        audio_data, sample_rate = librosa.load(full_audio_path, sr=CONFIG["sample_rate"])
        audio_data = nr.reduce_noise(y=audio_data, sr=sample_rate, prop_decrease=CONFIG["noise_reduction_strength"])
        harmonic, _ = librosa.effects.hpss(audio_data, margin=3.0)
        background_audio = audio_data - (harmonic * CONFIG["vocal_isolation_strength"])
        if CONFIG["audio_normalization"]:
            harmonic = librosa.util.normalize(harmonic)
            background_audio = librosa.util.normalize(background_audio) * CONFIG["background_reduction_factor"]
        vocals_path = os.path.join(self.temp_dir, "vocals.wav")
        background_path = os.path.join(self.temp_dir, "background.wav")
        sf.write(vocals_path, harmonic, sample_rate)
        sf.write(background_path, background_audio, sample_rate)
        return vocals_path, background_path, vocals_path, duration

    def transcribe_audio(self, audio_path: str) -> List[Dict]:
        result = self.whisper_model.transcribe(audio_path, language="en", word_timestamps=True, verbose=False)
        return [{"start": s["start"], "end": s["end"], "text": s["text"].strip()} for s in result["segments"]]

    def translate_text(self, text: str) -> str:
        inputs = self.tokenizer(text, return_tensors="pt", padding=True, truncation=True, max_length=512)
        output = self.translation_model.generate(**inputs, max_length=512, num_beams=4, early_stopping=True)
        return self.tokenizer.decode(output[0], skip_special_tokens=True).strip()

    def synthesize_arabic_speech(self, text: str, output_path: str) -> bool:
        speech_config = speechsdk.SpeechConfig(subscription=AZURE_SPEECH_KEY, region=AZURE_SPEECH_REGION)
        speech_config.speech_synthesis_voice_name = CONFIG["arabic_voice"]
        speech_config.set_speech_synthesis_output_format(speechsdk.SpeechSynthesisOutputFormat.Riff16Khz16BitMonoPcm)
        synthesizer = speechsdk.SpeechSynthesizer(speech_config=speech_config, audio_config=None)
        result = synthesizer.speak_text_async(text).get()
        if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
            with open(output_path, "wb") as f:
                f.write(result.audio_data)
            return True
        return False

    def _combine_arabic_audio(self, segments: List[Dict], total_duration: float) -> str:
        combined_path = os.path.join(self.temp_dir, "combined.wav")
        silence = AudioSegment.silent(duration=int(total_duration * 1000))
        for seg in segments:
            audio_seg = AudioSegment.from_wav(seg["path"])
            silence = silence.overlay(audio_seg, position=int(seg["start"] * 1000))
        silence.export(combined_path, format="wav")
        return combined_path

    def _combine_with_video(self, video_path: str, arabic_audio_path: str, background_path: str) -> str:
        output_path = os.path.join(self.temp_dir, f"dubbed_{uuid.uuid4().hex}.mp4")
        with VideoFileClip(video_path) as video:
            arabic_audio = AudioFileClip(arabic_audio_path)
            background_audio = AudioFileClip(background_path)
            final_audio = CompositeAudioClip([arabic_audio, background_audio])
            final_video = video.set_audio(final_audio)
            final_video.write_videofile(output_path, codec=CONFIG["video_codec"], audio_codec=CONFIG["audio_codec"], verbose=False, logger=None)
        return output_path

    def process_video(self, video_path: str) -> str:
        self.validate_input(video_path)
        vocals, bg, clean, duration = self.advanced_audio_separation(video_path)
        segments = self.transcribe_audio(clean)
        arabic_segments = []
        for i, seg in enumerate(segments):
            translated = self.translate_text(seg["text"])
            seg_audio_path = os.path.join(self.temp_dir, f"seg_{i}.wav")
            if self.synthesize_arabic_speech(translated, seg_audio_path):
                arabic_segments.append({"path": seg_audio_path, "start": seg["start"], "end": seg["end"]})
        final_audio = self._combine_arabic_audio(arabic_segments, duration)
        return self._combine_with_video(video_path, final_audio, bg)

@app.on_event("startup")
async def startup_event():
    global translator
    translator = EnhancedVideoTranslator()

@app.get("/")
async def root():
    return {"message": "Welcome to the Video Dubbing API", "version": "1.0.0"}

@app.post("/dub-video")
async def dub_video(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    if not translator:
        raise HTTPException(status_code=503, detail="Translator not initialized")
    supported = ', '.join(CONFIG["supported_formats"])
    if not file.filename.lower().endswith(tuple(CONFIG["supported_formats"])):
        raise HTTPException(status_code=400, detail=f"Unsupported file format. Supported: {supported}")
    temp_input = f"/tmp/{uuid.uuid4().hex}_{file.filename}"
    with open(temp_input, "wb") as f:
        content = await file.read()
        if len(content) > CONFIG["max_file_size"]:
            raise HTTPException(status_code=413, detail="File too large")
        f.write(content)
    loop = asyncio.get_event_loop()
    output = await loop.run_in_executor(executor, translator.process_video, temp_input)
    background_tasks.add_task(cleanup_files, temp_input, output)
    return FileResponse(output, media_type="video/mp4", filename=f"dubbed_{file.filename}")

def cleanup_files(*paths):
    for path in paths:
        if os.path.exists(path):
            os.remove(path)

if __name__ == "__main__":
    port = int(os.getenv("PORT", 8000))
    uvicorn.run("main:app", host="0.0.0.0", port=port, reload=False)
